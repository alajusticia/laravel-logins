...
class LoginsSessionHandler implements ExistenceAwareInterface, SessionHandlerInterface
{
    ...
    {
        $this->table = 'logins';
        $this->minutes = Config::get('sessions.lifetime');
        $this->container = $container;
        $this->connection = $container->make('db')->connection(Config::get('logins.database_connection'));
    }
    ...
    {
        $login = Login::withExpired()->where('session_id', $sessionId)->first();

        $this->exists = ! is_null($login);

        if (! $this->exists || $login->isExpired()) {
            return '';
        }

        return ! empty($login->session_payload) ? base64_decode($login->session_payload) : '';
    }
